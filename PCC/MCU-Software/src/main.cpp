...
void loop() {
  ...
  if(serial.available > 0)
  {
    ...
    if(message.command == SET_BASE_COLOR)
    {
    ...
    else if(message.command == SET_TEMP_COLOR)
    {
      uint8_t white = message.data[0];
      uint8_t red = message.data[1];
      uint8_t green = message.data[2];
    ...
    else if (message.command == SET_SERVO)
    {
      uint8_t which_servo = message.data[0];
      uint8_t angle = message.data[1];
    ...
    else if (message.command == RESET_VRC_PERIPH)
    {
      //digitalWrite(RST_PIN,LOW);
    }
    else if (message.command == CHECK_SERVO_CONTROLLER)
    {
